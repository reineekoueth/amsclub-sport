...
const router = createRouter({
  history: createWebHashHistory(), // ← hash mode pour éviter 404 sur Vercel
  routes
})
...
router.beforeEach((to, from) => {
  ...
  if (to.meta.requiresAuth && !token) return '/connexion'
  if (to.meta.requiresAdmin && utilisateur.role !== 'admin') return '/'
  return true
})
